...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--caption_file", 
          type=str, 
        default=None, 
        help="A JSON file mapping each image filename to its full caption (each caption must include your instance token).", 
    )
    parser.add_argument(
        "--class_prompt",
        type=str,
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        if caption_file is not None:
            with open(caption_file, "r") as f:
                # expects {"Shot_0012_v1.png":"caption…", …}
                self.captions_map = json.load(f)
        else:
            self.captions_map = None
    ...
    def __getitem__(self, index):
        ...
        # load the instance image tensor
        instance_image = self.pixel_values[index % self.num_instance_images]
        example["instance_images"] = instance_image

        # determine the prompt for this image
        if self.captions_map is not None:
            # use the filename to fetch the exact caption
            img_path = self.instance_image_paths[index % self.num_instance_images]
            filename = img_path.name
            # if the JSON lacks this filename, fall back to instance_prompt
            example["instance_prompt"] = self.captions_map.get(filename, self.instance_prompt)

        elif self.custom_instance_prompts:
            # legacy support: per-item custom prompts from a dataset
            ...
            # single-token fallback for all images
            example["instance_prompt"] = self.instance_prompt

        # optionally add class images + prompts for prior preservation
